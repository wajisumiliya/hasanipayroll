...
// ============================================================================
  ...
  Future<void> _showEditPayrollDialog() async {
    ...
    try {
      ...
      if (mounted) _message('Unable to load payroll editor: $e');
      return;
    }
    ...
    if (branchIds.isEmpty) {
      _message('No branches are available.');
      return;
    }

    DateTime month = selectedPayrollMonth;
    String branchId = branchIds.first;
    bool loading = false;
    String? tableError;
    final rowEmployees = <Map<String, dynamic>>[];
    final rowRecords = <String, Map<String, dynamic>?>{};
    final rowControllers = <String, Map<String, TextEditingController>>{};
    final remarksControllers = <String, TextEditingController>{};
    final savingIds = <String>{};
    final rowMessages = <String, String>{};
    final calculationRevisions = <String, int>{};
    final horizontalController = ScrollController();
    final verticalController = ScrollController();

    double number(String employeeId, String key) => double.tryParse(
          rowControllers[employeeId]![key]!.text.trim().replaceAll(',', ''),
        ) ?? 0;

    Future<void> recalculateRow(
      String employeeId, {
      StateSetter? refresh,
      bool debounce = true,
    }) async {
      final revision = (calculationRevisions[employeeId] ?? 0) + 1;
      calculationRevisions[employeeId] = revision;
      if (debounce) {
        await Future<void>.delayed(const Duration(milliseconds: 300));
      }
      if (calculationRevisions[employeeId] != revision ||
          !rowControllers.containsKey(employeeId)) {
        return;
      }
      try {
        final unpaidDeduction = number(employeeId, 'unpaid_deduction');
        final result =
            await AttendancePayrollService.calculateStatutoryContributions(
          employeeId: employeeId,
          basicSalary: number(employeeId, 'basic_salary'),
          salaryDeduction:
              number(employeeId, 'late_deduction') + unpaidDeduction,
        ...
        for (final key in automaticContributionFields) {
          rowControllers[employeeId]![key]!.text =
              result[key]!.toStringAsFixed(2);
        }
        rowMessages[employeeId] = result['contributions_applicable'] == 0
            ? 'Contributions set to RM0 (eligible wage below RM500).'
            : 'Contributions recalculated.';
      } catch (e) {
        rowMessages[employeeId] = 'Calculation failed: $e';
      }
      refresh?.call(() {});
    }

    void disposeRows() {
      for (final controllers in rowControllers.values) {
    ...
    Future<void> loadRows({StateSetter? refresh}) async {
      refresh?.call(() {
        loading = true;
        tableError = null;
      });
      ...
      try {
        ...
        final payrollById = <String, Map<String, dynamic>>{
          for (final row in List<Map<String, dynamic>>.from(responses[0]))
            _normalizeBranchValue(row['employee_id']): row,
        };
        ...
        rowRecords.clear();
        rowMessages.clear();
        calculationRevisions.clear();
        for (final employee in rowEmployees) {
          final id = _normalizeBranchValue(employee['employee_id']);
          final record = payrollById[id];
          final source = record ?? defaultsById[id] ?? <String, dynamic>{};
          rowRecords[id] = record;
          rowControllers[id] = {
            for (final field in fields)
              field.$1: TextEditingController(
                text: _payrollNumber(source[field.$1]).toStringAsFixed(2),
              ),
          };
        ...
        await Future.wait([
          for (final employee in rowEmployees)
            if (rowRecords[_normalizeBranchValue(employee['employee_id'])] ==
                null)
              recalculateRow(
                _normalizeBranchValue(employee['employee_id']),
                refresh: refresh,
                debounce: false,
              ),
        ]);
        ...
        loading = false;
        tableError = 'Unable to load payroll rows: $e';
        refresh?.call(() {});
      }
    }
        ...
        builder: (dialogContext) => StatefulBuilder(
          builder: (context, setDialogState) {
            Future<void> chooseMonth() async {
              ...
              await loadRows(refresh: setDialogState);
            }

            Future<void> saveRow(Map<String, dynamic> employee) async {
              final employeeId =
                  _normalizeBranchValue(employee['employee_id']);
              for (final field in fields) {
                final raw = rowControllers[employeeId]![field.$1]!
                    .text
                    .trim()
                    .replaceAll(',', '');
                ...
                if (value == null || value < 0) {
                  setDialogState(() {
                    rowMessages[employeeId] =
                        '${field.$2} must be a valid amount of 0 or more.';
                  });
                  return;
                }
              }
              setDialogState(() {
                savingIds.add(employeeId);
                rowMessages.remove(employeeId);
              });
              try {
                final existing = rowRecords[employeeId];
                ...
                final record = <String, dynamic>{
                  ...
                  'period': period,
                  for (final field in fields)
                    field.$1: number(employeeId, field.$1),
                  'new_ic_no': existing?['new_ic_no'] ?? employee['new_ic_no'],
                  'bank_code': existing?['bank_code'] ?? employee['bank_code'],
                  'bank_account':
                      existing?['bank_account'] ?? employee['bank_account'],
                  'remarks': remarksControllers[employeeId]!.text.trim().isEmpty
                      ? null
                      : remarksControllers[employeeId]!.text.trim(),
                ...
                rowRecords[employeeId] = Map<String, dynamic>.from(record);
                selectedPayrollMonth = month;
                await service.loadPayrollFromSupabase();
                if (!dialogContext.mounted) return;
                setDialogState(() {
                  savingIds.remove(employeeId);
                  rowMessages[employeeId] = 'Saved successfully.';
                });
                if (mounted) setState(() {});
              } catch (e) {
                if (!dialogContext.mounted) return;
                setDialogState(() {
                  savingIds.remove(employeeId);
                  rowMessages[employeeId] = 'Save failed: $e';
                });
              }
            }

            Widget amountCell(String employeeId, (String, String) field) {
              return SizedBox(
                width: 125,
                child: TextField(
                  controller: rowControllers[employeeId]![field.$1],
                  enabled: !savingIds.contains(employeeId),
                  keyboardType:
                      const TextInputType.numberWithOptions(decimal: true),
                  onChanged: (_) {
                    if (automaticContributionFields.contains(field.$1)) {
                      setDialogState(() {
                        calculationRevisions[employeeId] =
                            (calculationRevisions[employeeId] ?? 0) + 1;
                        rowMessages[employeeId] = 'Manual override entered.';
                      });
                    } else if (contributionBasisFields.contains(field.$1)) {
                      recalculateRow(employeeId, refresh: setDialogState);
                    }
                  },
                  decoration: InputDecoration(
                    isDense: true,
                    prefixText: 'RM ',
                    border: const OutlineInputBorder(),
                    fillColor: automaticContributionFields.contains(field.$1)
                        ? Colors.blue.shade50
                        : null,
                    filled: automaticContributionFields.contains(field.$1),
                  ),
                ),
              );
            }
            ...
            return AlertDialog(
              title: const Row(
                children: [
                  ...
                  SizedBox(width: 10),
                  Text('Edit Payroll - All Employees'),
                ],
              ),
              content: SizedBox(
                  ...
                  children: [
                    Wrap(
                      ...
                      children: [
                        ...
                        OutlinedButton.icon(
                          onPressed: loading ? null : chooseMonth,
                          icon: const Icon(Icons.calendar_month_outlined),
                          label: Text(DateFormat('MMMM yyyy').format(month)),
                        ),
                        Text('${rowEmployees.length} employee(s)'),
                        const Text(
                          'Blue fields are auto-calculated and remain editable.',
                        ),
                    ...
                    const SizedBox(height: 14),
                    Expanded(
                      child: rowEmployees.isEmpty && !loading
                          ? const Center(
                              child: Text('No active employees in this branch.'),
                            )
                          : Scrollbar(
                                    ...
                                    rows: [
                                      for (final employee in rowEmployees)
                                        DataRow(cells: [
                                          ...
                                          DataCell(SizedBox(
                                            width: 190,
                                            child: Text(rowMessages[
                                                    _normalizeBranchValue(
                                                  employee['employee_id'],
                                                )] ??
                                                (rowRecords[_normalizeBranchValue(
                                                            employee[
                                                                'employee_id'])] ==
                                                        null
                                                    ? 'New payroll'
              ...
              actions: [
                TextButton(
                  onPressed: !loading && savingIds.isEmpty
                      ? () => Navigator.pop(dialogContext)
                      : null,
                  child: const Text('Close'),
                ),
  ...
  // Kept temporarily as a rollback path while the row editor is adopted.
  // ignore: unused_element
  Future<void> _showLegacyEditPayrollDialog() async {
    List<Map<String, dynamic>> branches;
    List<Map<String, dynamic>> employees;
    try {
      final responses = await Future.wait([
