...
class SupabaseService {
  ...
  static Future<String> uploadEmployeePhoto({
    ...
    final publicUrl =
        client.storage.from('employee-photos').getPublicUrl(path);
    final versionedUrl =
  ...
  static Future<List<Map<String, dynamic>>> getEmployeesByBranch(
    ...
    try {
      ...
      return _mapList(response).where((employee) {
        ...
        final support = employee['is_support_staff'];
        if (support == true || support?.toString().toLowerCase() == 'true') {
          return false;
        }
        final isFrn =
