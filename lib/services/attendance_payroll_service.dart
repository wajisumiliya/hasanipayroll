...
class AttendancePayrollService {
  ...
  static Future<PayrollGenerationItem> generateEmployeePayroll({
    ...
    // 2. CUTI UMUM
    // ------------------------------------------------------------------------
    // Attendance calculation will be added later.
    // For now it is explicitly zero.
    // ------------------------------------------------------------------------

    const double cutiUmum = 0.0;
    ...
    if (epfCategory == 'reduced') {
      ...
      // Other categories continue to use the supplied EPF schedule.
    ...
    if (eisApplicable) {
      ...
      // EIS is not applicable for this employee.
  ...
  static Future<List<Map<String, dynamic>>>
    ...
    final response = await SupabaseService.client
        .from('attendance')
        .select(
          'employee_id,attendance_date,overtime_duration,'
          'ot_authorized,is_submitted',
        )
  ...
  static final List<_ContributionRow> _socsoFirstCategorySchedule = <_ContributionRow>[
